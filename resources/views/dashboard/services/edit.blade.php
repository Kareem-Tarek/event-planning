...
@section('title') {{__('admin/service.edit')}} ({{$model->name}}) @endsection

@section('content')

@component('components.breadcrumb')
@slot('breadcrumb_title')
<h3>{{__('admin/service.edit')}} ({{$model->name}})</h3>
@endslot
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="tab-content " id="pills-tabContent">
                <form action="{{route('services.update',$model->id)}}" method="post" id="alert-form">
                    @csrf
                    {{ method_field('put') }}
                    @include('dashboard.services.form')
                    <button class="btn btn-success mt-4 d-block me-auto" type="submit">{{__('admin/service.update')}}</button>
                </form>
            </div>
        </div>
