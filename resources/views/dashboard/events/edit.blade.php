...
@section('title') {{__('admin/event.edit')}} ({{$model->title}}) @endsection

@section('content')

    @component('components.breadcrumb')
        @slot('breadcrumb_title')
            <h3>{{__('admin/event.edit')}} ({{$model->title}})</h3>
        @endslot
        <li class="breadcrumb-item"><a href="{{route('events.index')}}">{{__('admin/event.events')}}</a> </li>
        <li class="breadcrumb-item active">{{__('admin/event.edit')}} ({{$model->title}})</li>
        @slot('bookmark')
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="tab-content " id="pills-tabContent">
                    <form action="{{route('events.update',$model->id)}}" method="post" id="alert-form" files="true" enctype="multipart/form-data">
                        @csrf
                        {{ method_field('put') }}
                        @include('dashboard.events.form')
                        <button class="btn btn-success mt-4 d-block me-auto" type="submit">{{__('admin/event.update')}}</button>
                    </form>
                </div>
            </div>
