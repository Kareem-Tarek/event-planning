...
@section('title') {{__('admin/governorate.edit')}} ({{$model->name}}) @endsection
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="tab-content " id="pills-tabContent">
                    <form action="{{route('governorates.update',$model->id)}}" method="post" id="alert-form">
                        @csrf
                        {{ method_field('put') }}
                        @include('dashboard.governorates.form')
                        <button class="btn btn-success mt-4 d-block me-auto" type="submit">{{__('admin/governorate.update')}}</button>
                    </form>
                </div>
            </div>
