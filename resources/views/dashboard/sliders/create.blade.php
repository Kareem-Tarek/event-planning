...
@section('title') {{__('admin/slider.create')}} @endsection

@section('content')

@component('components.breadcrumb')
@slot('breadcrumb_title')
<h3>{{__('admin/Slider.create')}}</h3>
@endslot
<li class="breadcrumb-item"><a href="{{route('sliders.index')}}">{{__('admin/slider.sliders')}}</a> </li>
<li class="breadcrumb-item active">{{__('admin/slider.create')}}</li>
...
<div class="col-sm-12 col-xl-6 xl-100">
    <div class="card">
        <div class="card-header pb-0">
            <h5>{{__('admin/slider.addSlider')}}</h5>
        </div>
        <div class="card-body">
            ...
            <div class="tab-content" id="pills-tabContent">
                <form action="{{route('sliders.store')}}" method="post" id="alert-form">
                    @csrf
                    @include('dashboard.sliders.form')
                    <button class="btn btn-success mt-4 d-block me-auto" type="submit">{{__('admin/home.add_new')}}</button>
                </form>
            </div>
        </div>
