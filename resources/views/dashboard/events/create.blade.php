...
@section('title') {{__('admin/event.create')}} @endsection

@section('content')

    @component('components.breadcrumb')
        @slot('breadcrumb_title')
            <h3>{{__('admin/event.create')}}</h3>
        @endslot
        <li class="breadcrumb-item"><a href="{{route('events.index')}}">{{__('admin/event.events')}}</a> </li>
        <li class="breadcrumb-item active">{{__('admin/event.create')}}</li>
