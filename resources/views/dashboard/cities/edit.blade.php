...
@section('title') {{__('admin/city.edit')}} ({{$model->name}}) @endsection

@section('content')

    @component('components.breadcrumb')
        @slot('breadcrumb_title')
            <h3>{{__('admin/city.edit')}} ({{$model->name}})</h3>
        @endslot
        <li class="breadcrumb-item"><a href="{{route('cities.index')}}">{{__('admin/city.cities')}}</a> </li>
        <li class="breadcrumb-item active">{{__('admin/city.edit')}} ({{$model->name}})</li>
        @slot('bookmark')
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="tab-content " id="pills-tabContent">
                    <form action="{{route('cities.update',$model->id)}}" method="post" id="alert-form">
                        @csrf
                        {{ method_field('put') }}
                        @include('dashboard.cities.form')
                        <button class="btn btn-success mt-4 d-block me-auto" type="submit">{{__('admin/city.update')}}</button>
                    </form>
                </div>
            </div>
